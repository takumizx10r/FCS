...
while n>0
    ...
    hold on
    semilogx(tau(1:FitRange),G(x,tau(1:FitRange)),'b-');
    hold off
    %     ylim([-0.1 1.1]);
    ...
    switch answer
        case 'Yes'
            ...
            dlgtitle = 'Initial values';
            dims = [1 35];
            definput = {num2str(Ni),num2str(Di),num2str(FitRange),num2str(dimension),num2str(movesum),num2str(Dz)};
            answer = inputdlg(prompt,dlgtitle,dims,definput);
            Ni=str2num(answer{1});
            Di=str2double(answer{2});
            FitRange=str2num(answer{3});
